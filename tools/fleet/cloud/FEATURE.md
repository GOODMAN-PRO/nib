...
5. Commit: `git add -A && git commit -m "Fxxx: <feature name>" -m "$(printf 'Co-Authored-By: Claude Opus 5.5 <noreply@anthropic.com>\nClaude-Session: https://claude.ai/code/session_01GpZ3Q12FGHi43JsYU8aD5F')"` (see COMMON.md for the exact trailer).
...
## V2ADOPT
The feature is built and green, but was written against contracts-v1 / design-v1 and carries workarounds that contracts-v2 and design-v2 now cover. 1) Merge origin/main. 2) Read docs/CONTRACTS.md "contracts-v2 changelog" and docs/DESIGN_SYSTEM.md "NibDesign v2 additions" and find every entry that names THIS feature (grep for the id, e.g. "F016") plus the lines for this feature in tools/fleet/contract-gaps.md marked `[v2: ...]`. 3) Replace each listed workaround with the v2 API (delete the local stand-in code, private keys, NSMapTables, NibApp.shared lookups, hand-made tokens/symbols, etc.), keeping behaviour identical or better; also drop any hand-dimming that NibDesign buttons now do themselves. 4) Keep/extend tests so the behaviour is still covered. 5) CI loop until green (only owned files). 6) Write {"status":"green","runUrl":"...","v2adopt":true} to /home/user/nib/tools/fleet/state/Fxxx.fixed.json (overwrite). Return what was replaced and anything v2 still doesn't cover.
