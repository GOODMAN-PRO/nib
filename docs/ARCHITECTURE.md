...
**Design system: `NibDesign` (reserved).** `NibKit/Sources/NibDesign` is a library module that is not a feature: no entry type, no commands, no forge-spec owner. Every module with a `ui` or `fullstack` feature depends on it (`Package.swift` puts `design` in that module's `deps`); core modules do not, and the app target gets it through the `NibKit` product. The scaffold ships it near-empty (`public enum NibDesign { public static let version = 0 }` plus an empty `Shaders/` folder kept by `.gitkeep`). A later design stage fills it with the locked design system: tokens, droplet/"liquid" components and Metal shaders (`.metal` files under `Shaders/`). From then on feature UI imports `NibDesign` for its styling and does not roll its own. It is architect-owned like `NibContracts`: feature agents never edit it, and missing pieces go through §16. It never imports a feature module. Every iOS 26 API it uses (Liquid Glass) sits behind `#available(iOS 26, *)` with an iOS 17 fallback. `Scripts/lint.py` allows `import NibDesign` next to `NibContracts`.
...
9. **UI:**
   ...
   - Use system materials (`.ultraThinMaterial`) for floating UI. Once the design stage has filled `NibDesign` (§3), take tokens, components and glass from it instead.
...
14. **Platform notes (iOS 17.0 target, iOS 26 SDK).**
    - Guard every iOS 26 symbol (Liquid Glass: `glassEffect`, `GlassEffectContainer`, `.glass` button styles, `UIGlassEffect`, and so on) with `if #available(iOS 26, *)` / `@available(iOS 26, *)`, and give it an iOS 17 fallback such as `.ultraThinMaterial`. Stay in Swift 5 language mode.
...
| Feature logic | `<Module>Tests`: geometry, parsers, layout, schedulers, codecs, with Harness for command tests and NibTesting fakes (`FakeRenderer`, `FakeRecognizer`, `FakeAIService`, `FakePDFService`, `FakeLockService`, `InMemorySecretStore`) for other features' services. Module test targets link only their own module. |
