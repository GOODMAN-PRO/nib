    ...
    public static func register(_ app: NibApp) {
        ...
        app.settings.declarePrefix(TimeKeeper.Keys.history, synced: true,
                                   ...
                                   schema: .obj(["kind": .str(choices: TimerKind.allCases.map { $0.rawValue }),
                                                 "label": .str(), "doc": .ref, "elapsed": .num(min: 0),
                                                 "laps": .arr(.anything())]))
...
/// The "time's up" alert for a countdown that ends while Nib is hidden or in the background. Behind a protocol so
/// tests use a fake; UNUserNotificationCenter crashes in hostless tests.
@MainActor
